...
class MainViewModel extends ChangeNotifier {
  final DouyinApiClient _apiClient = DouyinApiClient(http.Client());
  ...
  Future<void> _parseDouyin(String url) async {
    ...
    final items = response.data?.videoDataList ?? [];
    if (items.isEmpty) {
      // 尝试从image/video字段提取
      final data = response.data;
      if (data != null) {
        final videos = data.getAllVideoUrls();
        items.addAll(videos);
      }
    }
  ...
  Future<void> downloadItem(DownloadItem item) async {
    ...
    try {
      final subscription = _downloadManager.downloadFile(
        url: url,
